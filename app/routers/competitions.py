...
async def list_competitions(
    ...
    elif not has_event_type and venue_id is None:
        # Default "Compete" view: enterable competitions only — including when a
        # discipline filter is set, so a venue_hire row that carries a discipline
        # (e.g. "SJ Course Hire") can't leak into a discipline-filtered list. A
        # venue hand-off (venue_id set) shows everything on at that venue —
        # shows/clinics too — so it matches the map's event count instead of
